...
export default function () {
  ...
  function Generate(options: any) {
    ...
    if (options.useText) {
      ...
      for (let item of textStyles) {
        ...
        if (!fontFamilies.find((x: any) => x.value === item.fontName.family)) {
          fontFamilies.push({
            name: `${item.fontName.family.toLowerCase()}`,
            value: item.fontName.family,
        ...
        if (!letterSpacing.find((x: any) => x.originalValue === item.letterSpacing.value)) {
          ...
          letterSpacing.push({
            name: `${name}`,
            value: `${value}em`,
            originalValue: item.letterSpacing.value,
            cssVarPrefix: 'letter-spacing',
          })
        }
      }
    ...
    if (data.colors?.length <= 0 && data.fontStyles?.sizes?.length <= 0 && data.effects?.length <= 0) {
    ...
    figma.ui.postMessage({ type: 'generated', data: JSON.stringify(output) })
  }
  ...
  function CamelCaseToKebabCase(name: string) {
    return `${name.charAt(0)}${name.substr(1).replace(/([a-z0-9]|(?=[A-Z]))([A-Z])/g, '$1-$2')}` // camelCase to kebab-case
  }
  ...
  function GenerateCssVars(data: any) {
    ...
    if (data?.effects?.length > 0) {
      ...
      data.effects.forEach((effect: any) => {
        if (effect.blur === ``) {
          output += `--${effect.name}: ${effect.shadows};\n`
        } else {
          // blur exists, split effect into two
          output += `--${effect.name}-shadows: ${effect.shadows};\n`
          output += `--${effect.name}-blur: ${effect.blur};\n`
        }
      })
    }
