...
function Form({
  ...
  setOutput,
  handleExit,
}: { loading: boolean; setLoading: (loading: boolean) => void; setOutput: (data: string) => void; handleExit: any }) {
    ...
    <form onSubmit={handleSubmit} class="flex flex-col gap-4 justify-between h-full" style={{ marginBlockEnd: 0 }}>
      <div class="flex flex-col gap-4">
        <div class="flex flex-col gap-2">
          <label for="color-style" class="text-base">
            Color system
          </label>
          <select
            name="colorStyle"
        ...
        <div class="flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-4">
            <Toggle name="useColor" value={formState.useColor} onChange={updateValue}>
              <Text>Colors & Gradients</Text>
            </Toggle>
            <Toggle name="useText" value={formState.useText} onChange={updateValue}>
              <Text>Typography</Text>
            </Toggle>
            {formState.useText === true && (
              <div class="ml-6">
                <Toggle name="useRem" value={formState.useRem} onChange={updateValue}>
                  <Text>Use REM units?</Text>
                </Toggle>
              </div>
            )}
            ...
            <Toggle name="useSpacing" value={formState.useSpacing} onChange={updateValue}>
              <Text>Spacing</Text>
            </Toggle>
          </div>
          <div class="checkbox">
            <Toggle name="useEffect" value={formState.useEffect} onChange={updateValue}>
              <Text>Effects</Text>
            </Toggle>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-2">
          <label for="outputType" class="text-base">
            Output as
          </label>
          <div>
            <SegmentedControl
      ...
      <div class="flex flex-col gap-2">
        <Button id="generate" type="submit" loading={loading} disabled={disabled} fullWidth>
          {loading ? 'Generating' : 'Generate'}
        </Button>
        {(loading && (
          <Button type="button" secondary fullWidth onClick={handleCancel}>
            Cancel
          </Button>
        )) || (
          <Button type="button" secondary fullWidth onClick={handleExit}>
            Close
          </Button>
        )}
      </div>
